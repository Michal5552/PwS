...
namespace cashDispenserTest.MockRepositoriesTest
{
    ...
    public class MockPhysicalMoneyRepository_Test
    {
        private void initData()
        {
            ...
            {
                sw.WriteLine($"{DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt};0");
            }
        }
                    ...
                    switch (physicalMoneyInCurrency._Currency)
                    {
                        case Currency.PLN:
                        {
                            if (physicalMoneyInCurrency._Value !=
                                (DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt / PLN_currencyRate))
                            {
                        ...
                        {
                            if (physicalMoneyInCurrency._Value !=
                                (DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt / USD_currencyRate))
                            {
                                result = $"Bad value for {physicalMoneyInCurrency._Currency.ToString()}";
                            }
                        }
                        ...
                        {
                            if (physicalMoneyInCurrency._Value !=
                                (DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt / EUR_currencyRate))
                            {
                                result = $"Bad value for {physicalMoneyInCurrency._Currency.ToString()}";
                            }
                        }
                        ...
                        {
                            if (physicalMoneyInCurrency._Value !=
                                (DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt / GBP_currencyRate))
                            {
                                result = $"Bad value for {physicalMoneyInCurrency._Currency.ToString()}";
                            }
                        }
            ...
            decimal currencyRate, Currency currency, string expected)
        {
            ...
            if (result.Equals("OK"))
            {
                //Check value
                if ((physicalMoneyInCurrency._Value)
                    != (DbContextState.MockPhysicalMoneyRepository_Test_PhysicalMoney_txt / currencyRate))
                {
